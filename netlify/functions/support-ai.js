...
function orderAnswer(orders, text) {
  ...
  if (id) {
    ...
    return { text: `Buyurtma #${id}: ${statusLabel(o.status)}. Jami: ${money(o.totalUZS || o.amountUZS || 0)}.${o.shipping?.methodLabel ? ` Yetkazish: ${safeText(o.shipping.methodLabel,120)}.` : ''}`, category:'order' };
  }
  ...
  return { text: `Oxirgi buyurtmangiz #${oid}: ${statusLabel(o.status)}. Jami: ${money(o.totalUZS || o.amountUZS || 0)}.${orders.length>1?' Boshqa buyurtma bo‘yicha savol bo‘lsa uning raqamini yozing.':''}`, category:'order' };
}
...
exports.handler = async function(event){
  ...
  try{
    ...
    const snapshot={assistantName:cfg.assistantName,storePolicies:{contact:cfg.contactText,workHours:cfg.workHoursText,delivery:cfg.deliveryPolicy,payment:cfg.paymentPolicy,returns:cfg.returnsPolicy,extra:cfg.extraKnowledge},customer:{balanceUZS:Number(user.balanceUZS??user.balance??0)||0},recentOrders:orders.slice(0,5).map(o=>({orderId:String(o.id||o.orderId||''),status:statusLabel(o.status),totalUZS:Number(o.totalUZS||o.amountUZS||0)||0,shippingMethod:safeText(o.shipping?.methodLabel,100)})),matchingProducts:products.map(p=>({productId:String(p.id||''),name:productName(p),status:productStatus(p)||'catalog',stockQty:productQty(p),priceUZS:productPrice(p),description:safeText(p.description||p.desc||p.shortDescription||p.descriptionUz||'',700),fulfillmentType:safeText(p.fulfillmentType||'',60)})),activePromotions:promos.map(p=>({title:safeText(p.title,120),body:safeText(p.body,400)}))};
