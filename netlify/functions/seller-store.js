...
async function loadProducts(db,sellerId){let snap;try{snap=await db.collection('products').where('sellerId','==',sellerId).limit(600).get()}catch(_){return []}return snap.docs.map(d=>({id:d.id,data:d.data()||{}})).filter(x=>String(x.data.status||'').toLowerCase()==='approved'&&x.data.sellerActive!==false&&x.data.isActive!==false).map(x=>publicProduct(x.id,x.data)).sort((a,b)=>(b._created||0)-(a._created||0))}
...
exports.handler=async event=>{try{
  ...
  if(action==='public_store'){
    ...
    return C.json(200,{ok:true,store:publicStore(sellerId,seller,products.length),products,following});
  }
  ...
  if(action==='notifications_list'){
    const decoded=await decodedRequired(event);return C.json(200,{ok:true,...await notificationList(db,String(decoded.uid))});
  }
