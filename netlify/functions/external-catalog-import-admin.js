...
const EXTERNAL_SOURCES = {
  sahiy: { key: 'sahiy', label: 'Sahiy Market', shortLabel: 'Sahiy', currency: 'UZS', minDays: 15, maxDays: 30, hosts: ['sahiy.uz'] },
  uzum: { key: 'uzum', label: 'Uzum Market', shortLabel: 'Uzum', currency: 'UZS', minDays: 1, maxDays: 7, hosts: ['uzum.uz'] },
  '1688': { key: '1688', label: '1688', shortLabel: '1688', currency: 'CNY', minDays: 15, maxDays: 30, hosts: ['1688.com'] },
  pinduoduo: { key: 'pinduoduo', label: 'Pinduoduo', shortLabel: 'Pinduoduo', currency: 'CNY', minDays: 15, maxDays: 30, hosts: ['yangkeduo.com', 'pinduoduo.com'] },
};
...
function sourceSummary(item = {}) {
  ...
  return {
    ...
    sourcePlatform: detected.key, sourceLabel: detected.label, priceCurrency, priceValue,
...
function buildDescription(source = {}) {
  const label = source.sourceLabel || 'Tashqi market';
  const lines = [`${label} katalogidan buyurtma asosida olib kelinadigan mahsulot. Yetkazib berish muddati taxminiy.`];
...
function sanitizeDraft(raw = {}) {
  ...
  const cnyAuto = calculatePrice(source.priceCny).priceUzs; const localAuto = source.priceUzs || 0;
  const price = safeInt(raw.price, 0, 1e12, localAuto || cnyAuto); if (!price) throw Object.assign(new Error('OrzuMall sotuv narxini kiriting.'), { statusCode: 400 });
  return {
    ...
    weightKg: safeNumber(raw.weightKg, 0, 100000, 0), popularScore: safeInt(raw.popularScore, 0, 1e12, 50), tags: sanitizeTags(raw.tags?.length ? raw.tags : [detected.shortLabel, 'Tashqi katalog', 'Buyurtma asosida']),
    ...
    prepayRequired: raw.prepayRequired !== false,
  };
}
async function saveProduct(db, raw, actor) {
  ...
  const catalog = { ...buildExternalCustomerCatalog(source, draft.price), sourcePlatform: draft.detected.key, sourceLabel: draft.detected.label, sourceUrl: draft.sourceUrl, sourceUpstreamUrl: source.upstreamUrl || '', sourceItemId: draft.itemId };
  ...
  const externalMarket = {
    platform: draft.detected.key, sourceLabel: draft.detected.label, itemId: draft.itemId, url: draft.sourceUrl, upstreamUrl: source.upstreamUrl || '', originalTitle: source.title, priceCurrency: source.priceCurrency, priceValue: source.priceValue,
    priceCny: source.priceCny, priceUzs: source.priceUzs, moq: draft.moq, stock: draft.stock, unit: source.unit, sellerName: source.sellerName, sellerLocation: source.sellerLocation,
  ...
  const payload = {
    name: draft.name, name_ru: draft.name_ru, description: draft.description, price: draft.price, oldPrice: draft.oldPrice, weightKg: draft.weightKg, popularScore: draft.popularScore, currency: 'UZS', images: draft.images,
    ...
    tags: draft.tags, fulfillmentType: 'external_catalog', deliveryMinDays: Math.min(draft.deliveryMinDays, draft.deliveryMaxDays), deliveryMaxDays: Math.max(draft.deliveryMinDays, draft.deliveryMaxDays), prepayRequired: draft.prepayRequired,
...
function publicRow(doc) {
  ...
  return { id: doc.id, name: cleanText(p.name, 360), name_ru: cleanText(p.name_ru, 360), description: cleanText(p.description, 7000), price: safeInt(p.price, 0, 1e12, 0), oldPrice: safeInt(p.oldPrice, 0, 1e12, 0), image: images[0] || '', images,
    sourceUrl: ext.url, sourceUpstreamUrl: ext.upstreamUrl || '', sourcePlatform: ext.platform, sourceLabel: ext.sourceLabel, itemId: ext.itemId, priceValue: safeNumber(ext.priceValue, 0, 1e12, 0), priceCurrency: cleanText(ext.priceCurrency, 16), moq: safeInt(ext.moq, 1, 1e8, 1),
    fulfillmentType: cleanText(p.fulfillmentType, 32), isActive: p.isActive !== false, deliveryMinDays: safeInt(p.deliveryMinDays, 1, 365, sourceSpec(ext.platform)?.minDays || 15), deliveryMaxDays: safeInt(p.deliveryMaxDays, 1, 365, sourceSpec(ext.platform)?.maxDays || 30), tags: sanitizeTags(p.tags), weightKg: safeNumber(p.weightKg, 0, 100000, 0), popularScore: safeInt(p.popularScore, 0, 1e12, 0),
...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    if (action === 'list') { const snap = await db.collection('products').limit(600).get(); const products = snap.docs.filter(doc => isExternalProduct(doc.data() || {})).map(publicRow).sort((a, b) => b.updatedAtMs - a.updatedAtMs); return json(200, { products, pricing: pricingConfig(), supportedPlatforms: Object.values(EXTERNAL_SOURCES).map(x => ({ key: x.key, label: x.label })), rapidApiReady: rapidApi1688Ready(), rapidApiHost: rapidApi1688Ready() ? rapidApi1688Host() : '' }); }
