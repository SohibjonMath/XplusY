...
function renderOrders(){
  ...
  const cards=arr.length?arr.map(o=>{const st=normalizeStatus(o.status),note=o.statusNote||o.cancellation?.reason||o.returnRequest?.resolutionReason||"",age=orderAgeMs(o),aging=(["new","paid"].includes(st)&&age>=ORDER_ALERT_NEW_URGENT_MS)||(st==="packing"&&age>=ORDER_ALERT_PACKING_URGENT_MS),ageMin=Math.max(1,Math.floor(age/60000));return `<article class="order-card status-${esc(st)} ${aging?"is-aging":""}" data-order-detail="${esc(o.id)}"><div class="card-top"><div><div class="order-code">BUYURTMA • <span class="mono">#${esc(o.id)}</span></div><div class="card-title">${esc(orderOwner(o))}</div>${aging?`<span class="aging-tag"><i class="fa-solid fa-triangle-exclamation"></i>${ageMin} daqiqadan beri kutmoqda</span>`:""}</div>${statusPill(st)}</div><div class="card-grid"><div class="mini-info"><span>Telefon</span><b>${esc(orderPhone(o))}</b></div><div class="mini-info"><span>Summa</span><b>${esc(money(orderTotal(o)))}</b></div><div class="mini-info"><span>To‘lov</span><b>${esc(o.provider||o.paymentType||"—")}</b></div><div class="mini-info"><span>Vaqt</span><b>${esc(dateFmt(o.createdAt))}</b></div></div>${note?`<div class="order-note">${esc(note)}</div>`:""}${orderItemsPreviewHtml(o)}<div class="card-actions">${orderActionButtons(o)}<button class="btn btn-soft" data-order-detail-btn="${esc(o.id)}" type="button"><i class="fa-solid fa-eye"></i>Batafsil</button></div></article>`}).join(""):empty("fa-box-open",`${sm.label} buyurtma yo‘q.`);
  $("ordersList").innerHTML=head+cards;
}
function orderItemProduct(it={}){const id=String(it.productId||it.id||it.product?.id||"").trim();return id?(S.products.get(id)||{}):{}}
function orderItemImage(it={}){const p=orderItemProduct(it),rows=[it.image,it.productImage,it.imageUrl,it.thumbnail,it.img,it.product?.image,p.image,p.imageUrl,p.thumbnail,...(Array.isArray(it.images)?it.images:[]),...(Array.isArray(p.images)?p.images:[])];return String(rows.find(Boolean)||"").trim()}
function adminSourceInfo(it={}){const p=orderItemProduct(it),m=it.externalMarket&&typeof it.externalMarket==="object"?it.externalMarket:{},pm=p.externalMarket&&typeof p.externalMarket==="object"?p.externalMarket:{},platform=String(it.sourcePlatform||m.platform||p.sourcePlatform||pm.platform||"").trim().toLowerCase(),sourceUrl=String(it.sourceUrl||m.url||p.sourceUrl||pm.url||"").trim(),upstreamUrl=String(it.sourceUpstreamUrl||m.upstreamUrl||p.sourceUpstreamUrl||pm.upstreamUrl||"").trim(),fallback={sahiy:"Sahiy Market",uzum:"Uzum Market",1688:"1688",pinduoduo:"Pinduoduo"}[platform]||"Tashqi market",sourceLabel=String(it.sourceLabel||m.label||p.sourceLabel||pm.label||fallback).trim(),external=!!(platform||sourceUrl||upstreamUrl||it.externalCatalog||p.externalCatalog||String(p.fulfillmentType||"").toLowerCase()==="external_catalog");return{platform,sourceLabel,sourceUrl,upstreamUrl,external}}
function adminSourceBadgeHtml(it={}){const x=adminSourceInfo(it);if(!x.external)return '<span class="admin-source-badge local"><i class="fa-solid fa-store"></i>OrzuMall</span>';const cls=["sahiy","uzum","1688","pinduoduo"].includes(x.platform)?`src-${x.platform}`:"src-external";return `<span class="admin-source-badge ${esc(cls)}"><i class="fa-solid fa-globe"></i>${esc(x.sourceLabel)}</span>`}
function orderItemThumbHtml(it={}){const img=orderItemImage(it),name=it.title||it.name||it.productName||orderItemProduct(it)?.name||it.productId||"Mahsulot";return img?`<img class="order-item-thumb" src="${esc(img)}" alt="${esc(name)}" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.hidden=true;this.nextElementSibling.hidden=false"><span class="order-item-thumb-fallback" hidden><i class="fa-solid fa-image"></i></span>`:`<span class="order-item-thumb-fallback"><i class="fa-solid fa-image"></i></span>`}
function orderItemsPreviewHtml(o={}){const rows=Array.isArray(o.items)?o.items:[];if(!rows.length)return"";const shown=rows.slice(0,4),external=rows.map(adminSourceInfo).filter(x=>x.external),labels=[...new Set(external.map(x=>x.sourceLabel).filter(Boolean))];return `<div class="order-products-preview"><div class="order-preview-thumbs">${shown.map(it=>`<span class="order-preview-thumb-wrap">${orderItemThumbHtml(it)}</span>`).join("")}${rows.length>4?`<span class="order-preview-more">+${rows.length-4}</span>`:""}</div><div class="order-preview-copy"><b>${esc(String(rows.length))} ta mahsulot</b>${labels.length?`<span><i class="fa-solid fa-globe"></i>${esc(labels.join(" · "))}</span>`:`<span><i class="fa-solid fa-store"></i>OrzuMall ombori</span>`}</div></div>`}
function externalOrderItemHtml(it={}){const p=orderItemProduct(it),opts=it.externalOptions||it.selectedOptions||it.chinaOptions||{},variant=it.variantText||Object.values(opts).filter(Boolean).join(" / ")||[it.color,it.size,it.variant].filter(Boolean).join(" / ")||"—",src=adminSourceInfo(it),name=it.title||it.name||it.productName||p.name||it.productId||"Mahsulot";return `<article class="external-order-item"><div class="external-order-media">${orderItemThumbHtml(it)}</div><div class="external-order-main"><div class="external-order-badges">${adminSourceBadgeHtml(it)}</div><b>${esc(name)}</b><span>${esc(variant)} • ${esc(String(it.qty||1))} ta</span></div><div class="external-order-links">${src.sourceUrl?`<a href="${esc(src.sourceUrl)}" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-arrow-up-right-from-square"></i>${esc(src.sourceLabel)}da ochish</a>`:""}${src.upstreamUrl&&src.upstreamUrl!==src.sourceUrl?`<a href="${esc(src.upstreamUrl)}" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-link"></i>Asl manba</a>`:""}</div></article>`}
function orderItemsDetailHtml(o={}){const rows=Array.isArray(o.items)?o.items:[];return rows.length?`<div class="section-title"><span>Mahsulotlar</span></div><div class="external-order-items">${rows.map(externalOrderItemHtml).join("")}</div>`:""}
function orderDetails(o){const hist=Array.isArray(o.statusHistory)?o.statusHistory:[],st0=normalizeStatus(o.status);return `<div class="detail-row"><div class="mini-info"><span>Mijoz</span><b>${esc(orderOwner(o))}</b></div><div class="mini-info"><span>Telefon</span><b>${esc(orderPhone(o))}</b></div><div class="mini-info"><span>Jami</span><b>${esc(money(orderTotal(o)))}</b></div><div class="mini-info"><span>To‘lov</span><b>${esc(o.provider||"—")}</b></div></div><div class="mini-info"><span>Yetkazish</span><b>${esc(o.shipping?.methodLabel||o.shipping?.serviceLabel||"—")}</b></div>${orderAddress(o)?`<div class="order-note">${esc(orderAddress(o))}</div>`:""}${orderItemsDetailHtml(o)}${st0==="packing"?`<button class="btn btn-label label-wide" data-order-label="${esc(o.id)}" type="button"><i class="fa-solid fa-qrcode"></i>58×40 QR yorliq chiqarish</button>`:""}<div class="section-title"><span>Status tarixi</span></div><div class="timeline">${hist.length?hist.slice().reverse().map(h=>{const st=normalizeStatus(h.status);return `<div class="timeline-item status-${esc(st)}"><b>${esc(meta(st).label)} • ${esc(h.actorName||h.actorType||"Tizim")}</b><span>${esc(dateFmt(h.at,true))}${h.reason?`<br>${esc(h.reason)}`:""}</span></div>`}).join(""):`<div class="empty">Tarix hali yozilmagan.</div>`}</div><div class="card-actions">${orderActionButtons(o)}</div>`}
