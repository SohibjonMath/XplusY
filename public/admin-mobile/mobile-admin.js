...
function orderAlertCounts(){
  ...
  return {news,packing,urgentNew,urgentPacking,action:news.length+packing.length,urgent:urgentNew.length+urgentPacking.length};
}
...
function notifyOrderAlert(counts,reason="repeat"){
  ...
  const body=`Yangi: ${counts.news.length} ta. Yig‘ishda: ${counts.packing.length} ta.${counts.urgent?` Kechikayotgan: ${counts.urgent} ta.`:""}`;
...
function renderOrderAlertCenter(){
  ...
  $("alertNewHint").textContent=c.urgentNew.length?`${c.urgentNew.length} ta kechikyapti`:"Yig‘ishga oling";
  $("alertPackingHint").textContent=c.urgentPacking.length?`${c.urgentPacking.length} ta kechikyapti`:"Yetkazishga bering";
  root.querySelector(".alert-new")?.classList.toggle("urgent",!!c.urgentNew.length);root.querySelector(".alert-packing")?.classList.toggle("urgent",!!c.urgentPacking.length);
  const enable=$("orderAlertEnable");if(enable){enable.classList.toggle("is-on",S.alertsEnabled);enable.querySelector("span").textContent=S.alertsEnabled?"Signal yoqilgan":"Signalni yoqish";enable.querySelector("i").className=`fa-solid ${S.alertsEnabled?"fa-bell":"fa-bell-slash"}`}
  const msg=$("orderAlertMessage");if(msg){const span=msg.querySelector("span");if(!c.action)span.textContent="Hozir harakat talab qiladigan buyurtma yo‘q.";else if(alertSnoozed())span.textContent=`Signal ${alertTimeLeft()}ga tinchitilgan. Kartalar nazoratda qoladi.`;else if(c.urgent)span.textContent=`Diqqat: ${c.urgent} ta buyurtma belgilangan vaqtdan beri kutilmoqda.`;else span.textContent=`${c.action} ta buyurtma navbatdagi amalni kutmoqda. Signal har daqiqada takrorlanadi.`}
  const snooze=$("orderAlertSnooze");if(snooze)snooze.hidden=!c.action;
...
function onOrdersSnapshot(orders){
  ...
  const active=orders.filter(o=>["new","paid","packing"].includes(normalizeStatus(o.status)));
