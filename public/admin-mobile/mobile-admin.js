...
function externalOrderItemHtml(it={}){const opts=it.externalOptions||it.selectedOptions||it.chinaOptions||{},variant=it.variantText||Object.values(opts).filter(Boolean).join(" / ")||[it.color,it.size,it.variant].filter(Boolean).join(" / ")||"—",sourceUrl=String(it.sourceUrl||it.externalMarket?.url||"").trim(),sourceLabel=String(it.sourceLabel||it.externalMarket?.label||it.sourcePlatform||it.externalMarket?.platform||"Tashqi market").trim();return `<article class="external-order-item"><div><b>${esc(it.title||it.name||it.productId||"Mahsulot")}</b><span>${esc(variant)} • ${esc(String(it.qty||1))} ta</span></div>${sourceUrl?`<a href="${esc(sourceUrl)}" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-arrow-up-right-from-square"></i>${esc(sourceLabel)}da ochish</a>`:""}</article>`}
