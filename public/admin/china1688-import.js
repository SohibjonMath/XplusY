...
function syncReviewFields(src = {}, force = false) { if (state.reviewLocked && !force) return; if ($('reviewColors')) $('reviewColors').value = reviewColorText(previewColors(src)); if ($('reviewSizes')) $('reviewSizes').value = reviewSizeText(previewSizes(src)); state.reviewLocked = false; }
function reviewedSource(raw = state.source || {}) {
  const src = JSON.parse(JSON.stringify(raw || {}));
  const colors = parseReviewColors(); const sizes = parseReviewSizes();
  src.colorOptions = colors; src.sizeOptions = sizes;
  const others = (src.variantGroups || []).filter(g => !['color','size','spec'].includes(String(g?.type || '').toLowerCase()));
  src.variantGroups = [...(colors.length ? [{ id: 'review-colors', name: '颜色', type: 'color', options: colors }] : []), ...(sizes.length ? [{ id: 'review-sizes', name: '尺码', type: 'size', options: sizes }] : []), ...others];
  ...
  const key = (c, z) => `${cleanVariantLabel(c)}|||${cleanVariantLabel(z)}`;
  const oldMap = new Map(); oldRows.forEach(row => { const c = cleanVariantLabel(row.color || attrVariant(row,'color')); const z = cleanVariantLabel(row.size || attrVariant(row,'size')); if (c || z) oldMap.set(key(c,z), row); });
  let next=[];
  if (colors.length && sizes.length) colors.forEach(c => sizes.forEach(z => { const old=oldMap.get(key(c.name,z.name)) || {}; next.push({ ...old, id: old.id || `review-${next.length+1}`, name: `${c.name} / ${z.name}`, color:c.name, size:z.name, image:c.image || old.image || '', stock:Number(old.stock ?? stock ?? 0), priceCny:Number(old.priceCny || priceCny || 0), attributes:{ ...(old.attributes||{}), 颜色:c.name, 尺码:z.name } }); }));
  else if(colors.length) colors.forEach(c => { const old=oldMap.get(key(c.name,'')) || {}; next.push({ ...old,id:old.id||`review-${next.length+1}`,name:c.name,color:c.name,size:'',image:c.image||old.image||'',stock:Number(old.stock??stock??0),priceCny:Number(old.priceCny||priceCny||0),attributes:{...(old.attributes||{}),颜色:c.name} }); });
  else if(sizes.length) sizes.forEach(z => { const old=oldMap.get(key('',z.name)) || {}; next.push({ ...old,id:old.id||`review-${next.length+1}`,name:z.name,color:'',size:z.name,stock:Number(old.stock??stock??0),priceCny:Number(old.priceCny||priceCny||0),attributes:{...(old.attributes||{}),尺码:z.name} }); });
  else next=[];
  src.skuVariants=next; src.variants=next; src.imagesByColor={}; colors.forEach(c=>{ if(c.image) src.imagesByColor[c.name]=[c.image]; }); src.diagnostics={ ...(src.diagnostics||{}), mode:'admin-reviewed', reviewed:true, skuCount:next.length, groupCount:src.variantGroups.length };
  return src;
}
function applyReviewedVariants(showToast = true) { state.source = reviewedSource(state.source || {}); state.reviewLocked = true; renderVariants(state.source, { keepReview:true }); updateVariantReviewHint(state.source); if(showToast) toast('Variantlar yaratildi'); return state.source; }
function updateVariantReviewHint(src = state.source || {}) { const colors=parseReviewColors(), sizes=parseReviewSizes(), skus=(src.skuVariants||src.variants||[]); const hint=$('variantReviewHint'); if(!hint) return; hint.className=(colors.length||sizes.length)?'ok':'warn'; hint.textContent=(colors.length||sizes.length)?`${colors.length} rang · ${sizes.length} o‘lcham/model · ${skus.length} SKU tayyor`:'Variant aniqlanmadi. Kerak bo‘lsa qiymatlarni qo‘lda yozing.'; }
function autoPrepareVariants(src = state.source || {}) { syncReviewFields(src, true); const hasRows=parseReviewColors().length || parseReviewSizes().length; if(!hasRows){ updateVariantReviewHint(src); return src; } state.source=reviewedSource(src); state.reviewLocked=true; updateVariantReviewHint(state.source); return state.source; }
function renderVariants(src = {}, opts = {}) {
  ...
  $('variantList').innerHTML = `
    ...
    <div class="variant-group"><h5>${esc(src.genericSpecName ? 'O‘lcham / variantlar' : 'Razmerlar')}</h5><div class="variant-options">${optionCards(sizes, 'size')}</div></div>
    ${groups.filter(g => !['color', 'size', 'spec'].includes(g.type)).map(g => `<div class="variant-group"><h5>${esc(g.name)}</h5><div class="variant-options">${optionCards(g.options || [], 'other')}</div></div>`).join('')}
    <div class="sku-preview"><h5>SKU kombinatsiyalari</h5><div>${skus.length ? skus.slice(0, 14).map(v => `<span>${esc([v.color, v.size, v.name && ![v.color, v.size].filter(Boolean).includes(v.name) ? v.name : ''].filter(Boolean).join(' / ') || 'SKU')}${v.priceCny ? ` · ${esc(cny(v.priceCny))}` : ''}${v.stock ? ` · ${esc(v.stock)} ta` : ''}</span>`).join('') : '<em class="empty-variant">SKU aniqlanmadi</em>'}</div></div>`;
...
function applySource(src = {}, opts = {}) {
  state.reviewLocked = false; state.source = src; state.externalImages = sourceGallery(src); const images = state.externalImages;
...
async function apiPreview() {
  const url=extract1688Url($('sourceUrl').value); if(!url) return notice('API orqali olish uchun haqiqiy 1688 havolasini kiriting.','error');
  const btn=$('apiBtn'); setBusy(btn,true,'API’dan olinmoqda...'); notice('RapidAPI orqali strukturali SKU variantlari olinmoqda...','info');
  try { const d=await api({action:'apiPreview',sourceUrl:url}); const src=d.item||{}; state.editingId=''; $('editState').textContent='API orqali yangi qoralama'; $('extensionReady').classList.add('received'); $('extensionReady').innerHTML='<i class="fa-solid fa-bolt"></i><div><b>RapidAPI orqali strukturali mahsulot olindi</b><p>Asosiy rasmlar, variant guruhlari, SKU narxlari va qoldiqlarni tekshirib saqlang.</p></div>'; $('sourceUrl').value=url; applySource(src); notice(`${d.cached?'Keshdan':'RapidAPI’dan'} olindi: ${(src.galleryImages||src.images||[]).length} ta galereya rasmi · ${(src.variantGroups||[]).length} ta variant guruhi · ${(src.skuVariants||[]).length} ta SKU.`, 'success'); }
  catch(e){ notice(e.message==='RAPIDAPI_1688_KEY_MISSING'?'Netlify Environment Variables ichiga RAPIDAPI_1688_KEY ni kiriting. Kalitni HTML yoki frontend JavaScript fayliga yozmang.':'API import xatosi: '+e.message,'error'); }
  finally { setBusy(btn,false,'API orqali avto olish'); }
}
...
async function loadList() { if (!auth.currentUser) return; try { const d = await api({ action: 'list' }); state.products = d.products || []; state.pricing = d.pricing || state.pricing; const helper=$('apiHelper'); if(helper){helper.className='api-helper '+(d.rapidApiReady?'ready':'warn');helper.innerHTML=d.rapidApiReady?'<i class="fa-solid fa-circle-check"></i> RapidAPI server rejimi tayyor: linkdan strukturali variantlar olinadi.':'<i class="fa-solid fa-triangle-exclamation"></i> RAPIDAPI_1688_KEY hali Netlify’da kiritilmagan. Chrome importer zaxira rejim sifatida ishlaydi.';} renderList(); } catch (e) { toast(e.message); } }
...
$('apiBtn').addEventListener('click', apiPreview); $('manualBtn').addEventListener('click', manual); $('saveBtn').addEventListener('click', save); $('applyVariantReviewBtn').addEventListener('click', () => applyReviewedVariants(true)); $('clearVariantReviewBtn').addEventListener('click', () => { $('reviewColors').value=''; $('reviewSizes').value=''; state.reviewLocked=true; applyReviewedVariants(true); }); ['reviewColors','reviewSizes'].forEach(id => $(id).addEventListener('input', () => { state.reviewLocked=true; const hint=$('variantReviewHint'); hint.className='warn'; hint.textContent='O‘zgarish bor. “Variantlarni tayyorlash” tugmasini bosing yoki katalogga saqlang.'; })); $('refreshBtn').addEventListener('click', loadList); $('loginBtn').addEventListener('click', () => signInWithPopup(auth, provider).catch(e => toast(e.message))); $('authBtn').addEventListener('click', () => auth.currentUser ? signOut(auth) : signInWithPopup(auth, provider).catch(e => toast(e.message))); $('priceCny').addEventListener('input', () => { $('autoPriceLabel').textContent = money(calcAutoPrice($('priceCny').value)); }); $('savedGrid').addEventListener('click', e => { const edit = e.target.closest('[data-edit]'), normalize = e.target.closest('[data-normalize]'), archive = e.target.closest('[data-archive]'); if (edit) editRow(edit.dataset.edit); if (normalize) normalizeRow(normalize.dataset.normalize, normalize); if (archive) archiveRow(archive.dataset.archive); });
