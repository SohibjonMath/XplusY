...
function normColors(p){
  ...
  return arr.map(c=>{
    if(typeof c === "string") return {name:c, hex:null, image:null};
    return {name: c.name || c.label || "Color", hex: c.hex || c.color || null, image: c.image || c.img || c.imageUrl || null};
  });
}
...
function omQVVariantHtml(p){
  ...
  const parts=[];
  if(colors.length){
    parts.push(`<div class="qvVarGroup"><span>Ranglar</span><div class="qvSwatches">${colors.slice(0,12).map(c=>`<i class="qvSwatch ${c.image?"hasImage":""}" title="${escapeHtml(c.name)}" style="${c.hex?`--qvc:${escapeHtml(c.hex)}`:""}">${c.image?`<img src="${escapeHtml(c.image)}" alt="" loading="lazy" decoding="async">`:""}</i>`).join("")}${colors.length>12?`<b>+${colors.length-12}</b>`:""}</div></div>`);
  }
  if(sizes.length){
