...
function omCleanVariantLabel(value){
...
function omVariantRows(p){
  ...
  add(p?.variants); add(p?.skuVariants); add(p?.china1688?.skuVariants); add(p?.china1688?.variants);
  return rows;
}
function omVariantGroups(p){
...
function normColors(p){
  ...
  add(p?.colors); add(p?.colorOptions); add(p?.china1688?.colorOptions); add(omGroupByType(p,"color")?.options);
  const rows=omVariantRows(p);
  rows.forEach(v=>{
...
function normImages(p, sel){
  ...
  const colorMaps = [p?.imagesByColor, p?.china1688?.imagesByColor].filter(x=>x && typeof x === "object");
  for(const colorMap of colorMaps){
    const arr = colorMap[color] || colorMap[color?.toLowerCase?.()] || null;
...
function getVariantPricing(p, sel){
  ...
  if(marketVariants.length){
    const pick = (c,s)=> marketVariants.find(v=>
      (v?.color ?? null) === (c ?? null) &&
      (v?.size  ?? null) === (s ?? null)
    );
    const v =
  ...
  const sw = colors.length ? `
    <div class="optLine swatchesLine" aria-label="Rang">
      ${colors.map(c=>{
        ...
        const pic = c.image ? `<img src="${escapeHtml(c.image)}" alt="" loading="lazy" decoding="async">` : "";
        return `<button class="swatch ${active} ${c.image?"hasImage":""}" ${style} data-c="${escapeHtml(c.name)}" title="${escapeHtml(c.name)}">${pic}</button>`;
      }).join("")}
    </div>` : "";
...
function handleAddToCart(p, opts={}){
  ...
  const needsChoice = productNeedsVariantModal(p);
...
function renderVariantModal(){
  ...
  if(els.vColorRow) els.vColorRow.innerHTML = showColors ? colors.map(c=>{
    ...
    const pic = c.image ? `<img src="${escapeHtml(c.image)}" alt="" loading="lazy" decoding="async">` : "";
    return `<button class="vSwatch ${active} ${c.image?"hasImage":""}" ${bg} data-c="${escapeHtml(c.name)}" title="${escapeHtml(c.name)}" aria-label="${escapeHtml(c.name)}">${pic}</button>`;
  }).join("") : "";
...
function bindProductPage(p){
  ...
  root.querySelectorAll("[data-pp-size]").forEach(btn=>btn.addEventListener("click",(e)=>{
    e.preventDefault(); e.stopPropagation();
    const now=getSel(p); now.size=btn.getAttribute("data-pp-size")||null; selected.set(p.id,now); renderProductPage();
  }));
  ...
  const forceGalleryContain = ()=>{
    const isMobileGallery = window.matchMedia?.("(max-width: 768px)")?.matches === true;
    const isChinaGallery = omIsChina1688Product(p);
    if(galleryMainBtn){
      galleryMainBtn.style.setProperty("overflow", "hidden", "important");
      galleryMainBtn.style.setProperty("padding", "0", "important");
      if(isMobileGallery){
        galleryMainBtn.style.setProperty("position", "relative", "important");
        ...
        galleryMainBtn.style.setProperty("height", "min(calc(100vw - 20px), 390px)", "important");
        galleryMainBtn.style.setProperty("max-height", "390px", "important");
        galleryMainBtn.style.setProperty("aspect-ratio", "auto", "important");
        galleryMainBtn.style.setProperty("line-height", "0", "important");
        galleryMainBtn.style.setProperty("text-align", "center", "important");
      }else{
        galleryMainBtn.style.setProperty("display", "flex", "important");
        galleryMainBtn.style.setProperty("align-items", "center", "important");
        galleryMainBtn.style.setProperty("justify-content", "center", "important");
    ...
    if(galleryMainImg){
      ...
      // v166: imported 1688 supplier photos often contain generous white canvas.
      // Keep contain semantics, but enlarge the visible product carefully. Very
      // wide/tall photos receive a softer zoom so important edges stay visible.
      const nativeRatio = Number(galleryMainImg.naturalWidth || 0) > 0 && Number(galleryMainImg.naturalHeight || 0) > 0
      ...
      if(isMobileGallery){
        galleryMainImg.style.setProperty("position", "static", "important");
        galleryMainImg.style.setProperty("inset", "auto", "important");
        galleryMainImg.style.setProperty("top", "auto", "important");
        galleryMainImg.style.setProperty("right", "auto", "important");
        galleryMainImg.style.setProperty("bottom", "auto", "important");
        galleryMainImg.style.setProperty("left", "auto", "important");
        galleryMainImg.style.setProperty("width", isChinaGallery ? "100%" : "auto", "important");
        galleryMainImg.style.setProperty("height", isChinaGallery ? "100%" : "auto", "important");
        ...
        galleryMainImg.style.removeProperty("top");
        galleryMainImg.style.removeProperty("right");
        galleryMainImg.style.removeProperty("bottom");
        galleryMainImg.style.removeProperty("left");
        galleryMainImg.style.setProperty("width", "100%", "important");
        galleryMainImg.style.setProperty("height", "100%", "important");
        galleryMainImg.style.setProperty("max-width", "100%", "important");
        galleryMainImg.style.setProperty("max-height", "100%", "important");
        galleryMainImg.style.setProperty("margin", "auto", "important");
        galleryMainImg.style.setProperty("padding", isChinaGallery ? "2px" : "0", "important");
        galleryMainImg.style.setProperty("box-sizing", "border-box", "important");
      }
    }
  };
...
function omQVVariantHtml(p, interactive=false){
  ...
  if(colors.length){
    parts.push(`<div class="qvVarGroup"><span>${omTrHtml("Ranglar")}</span><div class="qvSwatches">${colors.slice(0,18).map(c=>{
      ...
      const tag=interactive?"button":"i";
      return `<${tag}${attrs} class="qvSwatch ${c.image?"hasImage":""}${active}" title="${escapeHtml(c.name)}" style="${c.hex?`--qvc:${escapeHtml(c.hex)}`:""}">${c.image?`<img src="${escapeHtml(c.image)}" alt="" loading="lazy" decoding="async">`:""}<small>${escapeHtml(c.name)}</small></${tag}>`;
...
async function omCxRunProductSearch(q,seq){try{let rows=omCxSearchState.cache.get(q);if(!rows){const out=await omCxApi('search_products',{query:q,limit:28});rows=Array.isArray(out.products)?out.products:[];omCxSearchState.cache.set(q,rows)}if(seq!==omCxSearchState.seq||q!==omCxSearchState.query)return;omCxMergeProducts(rows);omCxSearchStatus(`${rows.length} ta mos mahsulot katalog bo‘yicha tekshirildi.`,true);applyFilterSort()}catch(_){if(seq===omCxSearchState.seq)omCxSearchStatus(omTrText('Katalog qidiruvini yakunlab bo‘lmadi.'),false)}finally{if(seq===omCxSearchState.seq){omCxSearchState.loading=false;setTimeout(()=>{if(omCxSearchState.query===q)omCxSearchStatus('',false)},2200)}}}
function omScheduleProductSearch(raw){const q=norm(raw);if(q===omCxSearchState.query)return;omCxSearchState.query=q;omCxSearchState.seq+=1;clearTimeout(omCxSearchState.timer);if(q.length<2){omCxSearchState.loading=false;omCxSearchStatus('',false);return}omCxSearchState.loading=true;omCxSearchStatus(omTrText('Butun katalog bo‘yicha qidirilmoqda...'));const seq=omCxSearchState.seq;omCxSearchState.timer=setTimeout(()=>omCxRunProductSearch(q,seq),220)}
function omRememberViewedProduct(p){if(!p?.id)return;const sel=getSel(p),pricing=getVariantPricing(p,sel);const snap={id:String(p.id),name:String(p.name||p.title||'Mahsulot'),name_ru:String(p.name_ru||p.nameRu||''),name_en:String(p.name_en||p.nameEn||''),image:getCurrentImage(p,sel)||p.images?.[0]||p.image||'',price:Number(pricing.price||p.price||0),deliveryMinDays:omIsChinaOriginProduct(p)?20:Number(p.deliveryMinDays||1),deliveryMaxDays:omIsChinaOriginProduct(p)?20:Number(p.deliveryMaxDays||3),fulfillmentType:String(p.fulfillmentType||'stock'),originCountry:omIsChinaOriginProduct(p)?'CN':'UZ',sourcePlatform:String(p.sourcePlatform||p.externalMarket?.platform||''),viewedAt:Date.now()};const next=[snap,...omCxReadRecent().filter(x=>String(x.id)!==snap.id)].slice(0,18);omCxWriteRecent(next);setTimeout(()=>omRenderRecentShelf(),0);omCxRecommendations=[];setTimeout(()=>omRefreshRecommendations(true),250)}
function omCxDeliveryText(p={}){return omTrText(omIsChinaOriginProduct(p)?'Taxminan 20 kun':'Manzil tanlangandan keyin')}
function omCxMiniCardHtml(p={},badge=''){const id=String(p.id||''),name=omProductText(p,'name',p.name||'Mahsulot'),sel=getSel(p),price=Number(getVariantPricing(p,sel)?.price||p.price||0),img=getCurrentImage(p,sel)||p.images?.[0]||p.image||'./logo-256.webp',badgeText=badge?omTrText(badge):'';return `<article class="cxMiniCard" data-cx-product="${escapeHtml(id)}"><div class="cxMiniImg"><img src="${escapeHtml(img)}" alt="${escapeHtml(name)}" loading="lazy" decoding="async">${badgeText?`<span class="cxMiniBadge">${escapeHtml(badgeText)}</span>`:''}</div><div class="cxMiniBody"><div class="cxMiniName">${escapeHtml(name)}</div><div class="cxMiniPrice">${moneyUZS(price)}</div><div class="cxMiniDelivery"><i class="fa-solid fa-truck-fast"></i> ${escapeHtml(omCxDeliveryText(p))}</div></div></article>`}
...
function omRenderRecentShelf(){const section=document.getElementById('cxRecentSection'),rail=document.getElementById('cxRecentRail');if(!section||!rail)return;const rows=omCxReadRecent();section.hidden=!rows.length;rail.innerHTML=rows.map(x=>omCxMiniCardHtml({...x,...(findProductById(String(x.id))||{})},'Ko‘rilgan')).join('');omCxBindShelf(rail);try{window.OM_I18N?.notify?.(section)}catch(_e){}}
...
function omRenderRecommendationShelf(){const section=document.getElementById('cxRecommendSection'),rail=document.getElementById('cxRecommendRail');if(!section||!rail)return;section.hidden=!omCxRecommendations.length;rail.innerHTML=omCxRecommendations.map(x=>omCxMiniCardHtml(x,'Tavsiya')).join('');omCxBindShelf(rail);try{window.OM_I18N?.notify?.(section)}catch(_e){}}
...
document.getElementById('cxRecentClear')?.addEventListener('click',()=>{omCxWriteRecent([]);omRenderRecentShelf();toast(omTrText('Ko‘rilganlar tarixi tozalandi.'))});
document.getElementById('cxRecommendRefresh')?.addEventListener('click',()=>{omCxRecommendations=[];omRefreshRecommendations(true)});
window.addEventListener('om-i18n-updated',()=>{try{omRefreshCustomerExperience()}catch(_e){}});
